...
const resolveDashboardPath = (profile?: {
  ...
  if (profile?.default_dashboard === 'partner') {
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/fd862b92-28de-446b-a32b-39d1fc192b91', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'auth/callback/route.ts:resolveDashboardPath', message: 'Returning partner path', data: { reason: 'default_dashboard===partner' }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'A' }) }).catch(() => { });
    // #endregion
  ...
  if (profile?.partner_dashboard || profile?.user_type === 'partner') {
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/fd862b92-28de-446b-a32b-39d1fc192b91', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'auth/callback/route.ts:resolveDashboardPath', message: 'Returning partner path (fallback)', data: { reason: 'partner_dashboard||user_type===partner' }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'A' }) }).catch(() => { });
    // #endregion
    return '/dashboard/partner';
  }
...
export async function GET(request: Request) {
  ...
  if (user) {
    ...
    const { data: profileWithDefault, error: errorWithDefault } = await supabase
      .from('user_profiles')
      .select('onboarding_completed, user_type, admin_dashboard, partner_dashboard, broker_dashboard, default_dashboard')
      .eq('id', user.id)
      .maybeSingle();
    ...
    if (errorWithDefault?.message?.includes('default_dashboard does not exist')) {
      ...
      const { data: profileWithoutDefault, error: errorWithoutDefault } = await supabase
        .from('user_profiles')
        .select('onboarding_completed, user_type, admin_dashboard, partner_dashboard, broker_dashboard')
        .eq('id', user.id)
        .maybeSingle();
