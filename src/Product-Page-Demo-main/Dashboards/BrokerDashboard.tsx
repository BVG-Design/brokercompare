...
import OnboardingQuestionnaire from "../components/broker/OnboardingQuestionnaire";
...
export default function BrokerDashboard() {
  ...
  const { data: brokerProfile } = useQuery({
    queryKey: ['broker-profile', user?.email],
    queryFn: async () => {
      const profiles = await base44.entities.BrokerProfile.filter({ user_email: user.email });
  ...
  useEffect(() => {
    if (user && brokerProfile && !brokerProfile.onboarding_completed) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3">
                  ...
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-[#132847] flex items-center gap-2">
                        ...
                        Your Shortlist ({myShortlist.length})
                      </CardTitle>
                      {myShortlist.length > 0 && (
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => setActiveSection('saved-vendors')}
                        >
                          View All
                        </Button>
                      )}
                      ...
                      <>
                        <div className="space-y-3 mb-4">
                          {myShortlist.slice(0, 5).map(item => (
                            <Link
                              key={item.id}
                              to={createPageUrl(`VendorProfile?id=${item.vendor_id}`)}
                        ...
                        {myShortlist.length >= 2 && (
                          <Link to={createPageUrl(`CompareVendors?ids=${myShortlist.slice(0, 4).map(s => s.vendor_id).join(',')}`)}>
                            <Button className="w-full bg-[#05d8b5] hover:bg-[#04c5a6] text-white">
                ...
                <Card className="bg-gradient-to-br from-green-500 to-green-600 text-white">
                  <CardContent className="pt-6">
                    ...
                    <div className="space-y-3 bg-white/10 backdrop-blur-sm rounded-lg p-4">
                      ...
                      <Link to={createPageUrl('Community')} className="flex items-center justify-between p-3 bg-white/10 rounded-lg hover:bg-white/20 transition-colors">
                       <div>
            ...
            {activeSection === 'saved-vendors' && (
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-[#132847] flex items-center gap-2">
                      <Bookmark className="w-5 h-5" />
                      Saved Vendors ({myShortlist.length})
                    </CardTitle>
                    {selectedForComparison.length >= 2 && (
                      <Link to={createPageUrl(`CompareVendors?ids=${selectedForComparison.join(',')}`)}>
                        <Button className="bg-[#05d8b5] hover:bg-[#04c5a6] text-white">
                          <ArrowLeftRight className="w-4 h-4 mr-2" />
                ...
                <CardContent>
                  {myShortlist.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <h3 className="text-xl font-semibold text-gray-700 mb-2">No Saved Vendors</h3>
                      <p className="text-gray-600 mb-6">Start saving vendors to easily compare them later</p>
                      <Link to={createPageUrl('BrowseVendors')}>
                        <Button className="bg-[#132847] hover:bg-[#1a3a5f]">
                          Browse Vendors
                        </Button>
                    ...
                    <>
                      <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                        <p className="text-sm text-blue-900">
                          ...
                          Select 2-4 vendors to compare them side-by-side
                        </p>
                      </div>
                      <div className="space-y-3">
                        {myShortlist.map(item => (
                          <div 
                            ...
                            className={`flex items-center gap-3 p-4 border rounded-lg transition-all ${
                              selectedForComparison.includes(item.vendor_id)
                                ? 'border-[#05d8b5] bg-[#05d8b5]/5'
                                : 'hover:bg-gray-50'
                            }`}
                          >
                            <input
                              type="checkbox"
                              checked={selectedForComparison.includes(item.vendor_id)}
                              onChange={(e) => {
                                if (e.target.checked) {
                                  if (selectedForComparison.length < 4) {
                                    setSelectedForComparison([...selectedForComparison, item.vendor_id]);
                                  } else {
                                    toast.error("You can compare up to 4 vendors at once");
                                  }
                                } else {
                                  setSelectedForComparison(selectedForComparison.filter(id => id !== item.vendor_id));
                                }
                              }}
                              className="w-4 h-4 rounded border-gray-300 text-[#05d8b5] focus:ring-[#05d8b5]"
                            />
                            <div className="flex-1">
                              <p className="font-medium text-[#132847]">{item.vendor_name}</p>
                              {item.notes && (
                            ...
                            <Link to={createPageUrl(`VendorProfile?id=${item.vendor_id}`)}>
            ...
            {activeSection === 'settings' && (
              <Card>
                ...
                <CardContent>
                  {brokerProfile && brokerProfile.onboarding_completed && (
                    <div className="mb-8 p-6 bg-gray-50 rounded-lg space-y-4">
                      <h3 className="font-semibold text-lg text-[#132847]">Your Profile</h3>
                      {brokerProfile.aggregator && (
                        <div>
                          <p className="text-sm text-gray-600">Aggregator</p>
                          <p className="font-medium">{brokerProfile.aggregator === "Other" ? brokerProfile.aggregator_other : brokerProfile.aggregator}</p>
                        </div>
                      )}
                      {brokerProfile.team_size && (
                        <div>
                          <p className="text-sm text-gray-600">Team Size</p>
                          <p className="font-medium">{brokerProfile.team_size.replace(/_/g, ' ')}</p>
                        </div>
                      )}
                      {brokerProfile.top_priorities && brokerProfile.top_priorities.length > 0 && (
                        <div>
                          <p className="text-sm text-gray-600">Top Priorities</p>
                          <div className="flex flex-wrap gap-2 mt-1">
                            {brokerProfile.top_priorities.map((p, idx) => (
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              <Link to={createPageUrl('Community')} className="flex-1">
                <Button variant="outline" className="w-full">See Your Answers</Button>
              </Link>
              <Link to={createPageUrl('Community')} className="flex-1">
